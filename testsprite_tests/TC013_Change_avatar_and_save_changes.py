...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to the hash-based login route at http://localhost:57362/#/login to load the SPA and reveal interactive elements.
        await page.goto("http://localhost:57362/#/login", wait_until="commit", timeout=10000)
        
        # --> Assertions to verify final state
        frame = context.pages[-1]
        await expect(frame.locator("xpath=//img[contains(@src,'/avatars/') or contains(@alt,'Selected avatar')]").first).to_be_visible(timeout=3000)
