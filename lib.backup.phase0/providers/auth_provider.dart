...
class AuthProvider extends ChangeNotifier {
  ...
  static const _pendingPathIdKey = "cooklevel_pending_path_id";
  static const _pendingPathTitleKey = "cooklevel_pending_path_title";
  ...
  /// ==============================
  /// PENDING PATH - Para guardar path seleccionado antes de login
  /// ==============================
  Future<void> savePendingPath(String pathId, String pathTitle) async {
    await _storage.write(key: _pendingPathIdKey, value: pathId);
    await _storage.write(key: _pendingPathTitleKey, value: pathTitle);
  }

  Future<Map<String, String>?> getPendingPath() async {
    final pathId = await _storage.read(key: _pendingPathIdKey);
    final pathTitle = await _storage.read(key: _pendingPathTitleKey);

    if (pathId != null && pathId.isNotEmpty) {
      return {'pathId': pathId, 'pathTitle': pathTitle ?? ''};
    }
    return null;
  }

  Future<void> clearPendingPath() async {
    await _storage.delete(key: _pendingPathIdKey);
    await _storage.delete(key: _pendingPathTitleKey);
  }
}
