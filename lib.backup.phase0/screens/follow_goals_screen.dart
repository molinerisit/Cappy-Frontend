...
class _FollowGoalsScreenState extends State<FollowGoalsScreen> {
  ...
  Future<void> _selectPath(String pathId, String pathTitle) async {
    ...
    if (!authProvider.isAuthenticated) {
      // Si NO está autenticado, guardar el path seleccionado y navegar a login
      await authProvider.savePendingPath(pathId, pathTitle);

      if (!mounted) return;

      // Navegar a login, indicando que debe regresar después del login
      Navigator.of(context).pushReplacementNamed('/login');
      return;
    }

    // Si SÍ está autenticado, cambiar el path y navegar al tree
    setState(() => _isChangingPath = true);
    try {
      await ApiService.changeCurrentPath(pathId);
      if (!mounted) return;

      // Navega al árbol del camino pasando el pathId y title
              ...
              ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return GestureDetector(
                    onTap: _isChangingPath
                        ? null
                        : () => _selectPath(pathId, title),
                    child: Card(
                      elevation: 4,
